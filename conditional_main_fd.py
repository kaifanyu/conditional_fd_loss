...
def get_fd_train_step(model_wo_ddp, judges, sampling_args, args, tokenizer=None,
    ...
    def fd_train_step(lambda_cond_eff=None, alpha_eff=None, diag=False):
        ...
        clip_feats = None
        p_term_loss = None   # weighted CLIP attraction term (kept for grad diagnostics)
        q_term_loss = None   # weighted q_phi repulsion term (kept for grad diagnostics)
        if clip_classifier is not None:
            ...
            if qphi_trainer is not None:
                if getattr(qphi_trainer, "takes_images", False):
                    # EOT on the repulsion path too — same anti-adversarial guard
                    # the p_clip term gets, so the generator can't cheaply drive
                    # log q(c|x) down with a single-view adversarial texture.
                    log_q_c, q_diag = qphi_trainer.repulsion_with_diag(
                        sampled, y, eot_views=eot_views, noise_std=eot_noise, crop_min=eot_crop_min)
                else:
                    # legacy linear head on frozen features (operates on features, no EOT)
                    clip_feats = clip_classifier.image_features(sampled)
                    log_q_c, q_diag = qphi_trainer.repulsion_with_diag(clip_feats, y)
                q_term_loss = lambda_cond_eff * alpha_eff * log_q_c.mean()
                loss = loss + q_term_loss  # mult by lambda_cond_eff * alpha_eff
                # no floor: log_q_c.mean() == q_diag["logq_raw"] (the repulsion signal)
                loss_dict.update(q_diag)              # q_phi posterior reads (incl. logq_raw)
                loss_dict["q_loss_weighted"] = float(q_term_loss.detach())

        # -- per-term gradient diagnostics on the generated pixels (log-steps only).
        # Loss *values* are not comparable across terms (FD is self-normalized, p/q
        # are not), so we read who actually drives the update from grad-w.r.t.-image
        # norms; the cosines expose p/q cancellation and conflict with the FD pull.
        if diag:
            def _grad_x(term):
                if term is None:
                    return None
            ...
            g_fd, g_p, g_q = _grad_x(fd_term), _grad_x(p_term_loss), _grad_x(q_term_loss)
            def _norm(g):
            ...
            loss_dict["grad_x_q"] = _norm(g_q)
            loss_dict["cos_update_p_q"] = _cos(g_p, g_q)
            loss_dict["cos_update_fd_p"] = _cos(g_fd, g_p)
            loss_dict["cos_update_fd_q"] = _cos(g_fd, g_q)
...
def pretrain_qphi(qphi_trainer, clip_classifier, model_wo_ddp, sampling_args,
                  args, tokenizer=None):
    """Pre-fit q_phi on the (frozen) base model's 1-step samples BEFORE the
    generator loop, so q_phi tracks the generator from step 0 instead of taking
    ~18k online steps to catch up. This is what makes the alpha warmup meaningful:
    without it, alpha ramps to full (~step 2000) while q_phi is still at chance
    and — with the floor removed — feeds a garbage repulsion gradient.

    Two stages, mirroring the online observe()/train() exactly:
      1. fill the replay buffer using the SAME 1-step sampler the loop uses
         (sample_images_with_grad, no_grad, identical [-1,1]->[0,1] post). NOT
         model.generate(): that multi-step/CFG path is sharper than the 1-step
         outputs q_phi will see online and would re-introduce a train/fresh gap.
      2. run qphi_pretrain_steps SGD steps on the head/adapters vs. that buffer.

    Note: the base model's 1-step samples are blurry (low class signal — even
    CLIP's p(c|x) starts near uniform), so expect modest pretrain accuracy; the
    point is a calibrated warm start on the right input domain, not high acc.
    Per-rank buffers fill in lockstep (same batch/cap); train() AVG-syncs grads.
    Caller skips this on resume (q_phi already trained)."""
    n = int(getattr(args, "qphi_pretrain_steps", 0))
    if qphi_trainer is None or n <= 0:
        return
    takes_images = getattr(qphi_trainer, "takes_images", False)
    cap = qphi_trainer.buffer.capacity

    # stage 1: fill the buffer (no grad — just collecting images)
    model_wo_ddp.eval()
    logger.info(f"[q_phi/pretrain] filling buffer to {cap} with 1-step base samples ...")
    with torch.no_grad():
        while qphi_trainer.buffer.count < cap:
            z = torch.randn(args.batch_size, args.input_channels, args.input_size,
                            args.input_size, device="cuda") * args.noise_scale
            y = torch.randint(0, args.num_classes, (args.batch_size,), device="cuda")
            sampled = model_wo_ddp.sample_images_with_grad(z, y, sampling_args=sampling_args)
            if tokenizer is not None:
                sampled = tokenizer.decode(tokenizer.denormalize_z(sampled))
            sampled = (sampled * 0.5 + 0.5).clamp(0, 1)  # [-1,1] -> [0,1]
            qphi_trainer.observe(sampled if takes_images
                                 else clip_classifier.image_features(sampled), y)

    # stage 2: SGD on q_phi (needs grad enabled — train() calls loss.backward())
    logger.info(f"[q_phi/pretrain] buffer filled ({qphi_trainer.buffer.count}); "
                f"running {n} SGD steps ...")
    for it in range(1, n + 1):
        m = qphi_trainer.train(1)
        if it % 100 == 0 or it == n:
            qy = m.get("qphi/q_prob_y_buf", float("nan"))
            logger.info(f"[q_phi/pretrain] {it}/{n} ce={m['qphi/ce']:.3f} "
                        f"acc={m['qphi/acc']:.3f} q_prob_y_buf={qy:.4f}")
    model_wo_ddp.train()
...
def train_and_evaluate(args):
    ...
    # -- q_phi pretrain (fresh start only; resumed q_phi is already warm) --
    if qphi_trainer is not None and qphi_state is None:
        pretrain_qphi(qphi_trainer, clip_classifier, model_wo_ddp,
                      sampling_args, args, tokenizer=tokenizer)
    ...
    for step, _ in metric_logger.log_every(
        ...
        # per-term grad diagnostics cost 3 extra partial backwards through the
        ...
        loss, loss_dict, clip_feats, y_gen = fd_train_step(
            lambda_cond_eff=lambda_t, alpha_eff=alpha_t, diag=diag)
...
def get_args_parser():
    ...
    parser.add_argument("--qphi_pretrain_steps", type=int, default=0,
                        help="SGD steps to pre-fit q_phi on the frozen base model's "
                             "1-step samples BEFORE the generator loop. Fills the "
                             "replay buffer then trains, so q_phi tracks the "
                             "generator at step 0 instead of taking ~18k steps to "
                             "catch up (makes the alpha warmup meaningful). 0 "
                             "disables; skipped on resume.")
