...
class FeatureQueue(torch.nn.Module):
    ...
    def _snapshot(self, buf: torch.Tensor, new: torch.Tensor) -> torch.Tensor:
        ...
        snap.index_copy_(0, self._replacement_indices(new.shape[0]), new)
        return snap
    ...
    def enqueue(self, new_feats: torch.Tensor):
        ...
        indices = self._replacement_indices(n)
        self.feats.index_copy_(0, indices, new_det)
        self.ptr.add_(n).remainder_(self.size)

    # -- Internal helpers ------------------------------------------------------

    def _get_evicted_feats(self, n: int) -> torch.Tensor:
        """Return features that will be overwritten by the next enqueue of size n."""
        return self.feats.index_select(0, self._replacement_indices(n))

    def _replacement_indices(self, n: int) -> torch.Tensor:
        """Tensor indices for the next circular-buffer replacement region.

        Keeping the pointer on device avoids ``Tensor.item()`` graph breaks and
        stale pointer specialization when queue operations run under
        ``torch.compile``.
        """
        return (
            torch.arange(n, device=self.feats.device, dtype=torch.long)
            + self.ptr[0]
        ).remainder(self.size)
