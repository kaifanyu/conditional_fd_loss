...
def _to_cpu(obj):
    """Recursively snapshot tensors into independent CPU storage.

    ``Tensor.cpu()`` may return the original tensor when it already lives on
    CPU.  Checkpoint assembly can then alias live optimizer/queue state while
    an asynchronous save is in flight.  ``copy=True`` makes the snapshot
    immutable with respect to subsequent mutations of either CPU or CUDA
    source tensors.
    """
    if isinstance(obj, torch.Tensor):
        return obj.detach().to(device="cpu", copy=True)
...
def ckpt_resume(
    ...
    model_ema=None,
    extra_keys=None,
    expected_experiment_fingerprint=None,
):
    ...
    if args.resume_from or args.auto_resume:
        if args.resume_from is None:
            ...
            def _saved_step(path):
                try:
                    return int(os.path.basename(path).rsplit("_", 1)[-1].split(".")[0])
                except ValueError:
                    return -1

            # Prefer the greatest saved optimization step.  Modification time
            # can be misleading after copies, restores, or asynchronous writes.
            checkpoints = sorted(
                checkpoints, key=lambda path: (_saved_step(path), os.path.getmtime(path))
            )
        ...
        if args.resume_from and os.path.exists(args.resume_from):
            ...
            if expected_experiment_fingerprint is not None:
                saved_fingerprint = checkpoint.get("experiment_fingerprint")
                if saved_fingerprint != expected_experiment_fingerprint:
                    raise RuntimeError(
                        "Refusing to resume an incompatible checkpoint: "
                        f"expected experiment fingerprint {expected_experiment_fingerprint!r}, "
                        f"found {saved_fingerprint!r} in {args.resume_from}"
                    )
