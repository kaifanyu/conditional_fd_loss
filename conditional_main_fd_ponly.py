...
from classifier_ensemble import ClassifierEnsemble, ProbeClassifier  # multi-classifier p(c|x)
from vlm_judge import BinaryVLMJudge  # differentiable binary Yes/No VLM critic
...
def get_fd_train_step(model_wo_ddp, judges, sampling_args, args, tokenizer=None,
                      clip_classifier=None, probe=None):
    ...
    def fd_train_step(lambda_cond_eff=None, diag=False):
        ...
        # A large VLM is evaluated before the FD judge graphs are built. Its
        # implementation takes image-space VJPs in microbatches and injects an
        # exact first-order surrogate, so the VLM activations are already gone
        # when the three FD feature extractors run.
        p_term_loss = None
        is_vlm = bool(getattr(clip_classifier, "is_vlm_judge", False))
        if is_vlm:
            lambda_value = (float(lambda_cond_eff.detach()) if torch.is_tensor(lambda_cond_eff)
                            else float(lambda_cond_eff))
            if lambda_value > 0.0:  # skip the expensive VLM during cond warmup
                l_cond, log_p_c = clip_classifier.conditional_loss(
                    sampled, y, eot_views=eot_views, noise_std=eot_noise,
                    crop_min=eot_crop_min)
                p_term_loss = lambda_cond_eff * l_cond
                loss = loss + p_term_loss
                loss_dict.update(l_cond=float(l_cond.detach()), logp_c=float(log_p_c.detach()),
                                 p_loss_weighted=float(p_term_loss.detach()))
        ...
        # -- legacy classifier correction: -E[log p(c|x)] --
        if clip_classifier is not None and not is_vlm:
            # log p(c|x):
            ...
            p_term_loss = lambda_cond_eff * l_cond
            loss = loss + p_term_loss  # mult with lambda cond eff and add to loss
        ...
        if diag:
            ...
            if is_vlm:
                # Keep the historical key for plotting compatibility while
                # publishing an unambiguous name for new VLM runs.
                loss_dict["grad_x_vlm"] = loss_dict["grad_x_p"]
...
def train_and_evaluate(args):
    ...
    # -- conditional correction (zero-shot / supervised p(c|x)) --
    clip_classifier = None
    probe = None
    if args.lambda_cond > 0:
        if args.cond_vlm_model:
            if args.cond_classifiers:
                raise ValueError(
                    "--cond_vlm_model replaces the classifier ensemble; do "
                    "not also pass --cond_classifiers"
                )
            if args.compile:
                raise ValueError(
                    "--cond_vlm_model is incompatible with --compile: the "
                    "VLM judge performs eager processor calls and explicit "
                    "image VJPs"
                )
            clip_classifier = BinaryVLMJudge(
                args.cond_vlm_model,
                num_classes=args.num_classes,
                ...
                logp_cap=args.cond_target_logp,
                question_template=args.vlm_prompt_template,
                attn_implementation=args.vlm_attn_implementation,
            )
        elif args.cond_classifiers:
            clip_classifier = ClassifierEnsemble(
                args.cond_classifiers,
                num_classes=args.num_classes,
                device="cuda",
                dtype=args.clip_dtype,
                default_cap=args.cond_target_logp,
                combine=args.cond_combine,
                members_per_step=args.cond_members_per_step,
                clip_kwargs=dict(
        ...
        if getattr(clip_classifier, "is_vlm_judge", False):
            if args.vlm_skip_smoke:
                ...
                logger.info(
                    f"[VLMJudge] smoke: mean binary log p(correct)="
                    f"{smoke['mean_logp_per_class']:.3f} "
                    f"(uniform {smoke['uniform_ref']:.3f}), p(correct)="
                    f"{smoke['p_correct']:.3f}")
            logger.info(
                f"[VLMJudge] one frozen VLM '{args.cond_vlm_model}', "
                f"lambda_cond={args.lambda_cond} | mode={args.vlm_question_mode} "
                f"samples/step={args.vlm_samples_per_step or 'all'} "
                f"microbatch={args.vlm_microbatch} dtype={args.vlm_dtype} "
            ...
            smoke = clip_classifier.smoke_test_uniform(image_size=args.input_size)
            logger.info(f"[CLIPClassifier] smoke: mean log p/class="
                        f"{smoke['mean_logp_per_class']:.3f} "
                        f"(uniform ref {smoke['uniform_ref']:.3f})")
            logger.info(f"[CLIPClassifier] L_cond enabled, lambda={args.lambda_cond} "
                        f"| warmup={args.cond_warmup_steps} ramp={args.cond_ramp_steps} "
                        f"| eot={args.clip_eot_views} noise={args.clip_eot_noise_std} "
                        f"crop_min={args.clip_eot_crop_min} "
                        f"| cap={args.cond_target_logp} "
                        f"scale={args.clip_logit_scale or 'native'}")
    else:
        logger.info("[CLIPClassifier] lambda_cond=0 -> conditional correction disabled")

    if args.cond_probe and args.lambda_cond > 0:
        ...
        logger.info("[Probe] held-out resnet50 IMAGENET1K_V2 (the "
                    ...
                    "logp at diag steps — never in the loss")
    ...
    fd_train_step = get_fd_train_step(
        ...
        clip_classifier=clip_classifier, probe=probe,
    )
    ...
    for step, _ in metric_logger.log_every(
        ...
        loss, loss_dict = fd_train_step(lambda_cond_eff=lambda_t, diag=diag)
...
def get_args_parser():
    ...
    # -- anti-adversarial guards for the one-sided L_cond --
    ...
    # -- multi-classifier ensemble for log p(c|x) (classifier_ensemble.py) --
    parser.add_argument("--cond_classifiers", type=str, default="",
                        help="comma-separated ensemble spec 'kind:name[:k=v]*', kind in "
                             "{clip,timm}, options w/cap/T (+ scale/pre for clip). Empty "
                             "= single CLIP via --clip_model_name (legacy path). E.g. "
                             "'clip:ViT-L-14:cap=-0.69,"
                             "timm:convnext_base.fb_in22k_ft_in1k:cap=-0.69,"
                             "timm:deit3_base_patch16_224.fb_in22k_ft_in1k:cap=-0.69'")
    parser.add_argument("--cond_combine", type=str, default="mean",
                        choices=["mean", "min"],
                        help="mean: weighted mean of per-member capped log p (product "
                             "of experts). min: all gradient to the weakest member")
    parser.add_argument("--cond_members_per_step", type=int, default=0,
                        help="round-robin K members per step (0 = all every step). "
                             "K>=2 keeps a cross-paradigm consensus per step at "
                             "reduced cost")

    # -- single differentiable VLM critic (paper-style binary Yes/No loss) --
    parser.add_argument("--cond_vlm_model", type=str, default="",
                        help="Hugging Face VLM model id or local checkpoint path. When "
                             "set, replaces --cond_classifiers/the legacy CLIP scorer "
                             "with one frozen differentiable Yes/No VLM critic. This is "
                             "a PyTorch VLM, not the vLLM inference server.")
    parser.add_argument("--vlm_dtype", type=str, default="bf16",
                        choices=["bf16", "fp16", "fp32"],
                        help="frozen VLM parameter/compute dtype")
    parser.add_argument("--vlm_microbatch", type=int, default=1,
                        help="images per VLM forward. Detached-image VJPs are computed "
                             "per microbatch and injected into the generator graph, so "
                             "VLM activations can be released immediately")
    parser.add_argument("--vlm_samples_per_step", type=int, default=4,
                        help="round-robin-selected local generated images scored by the "
                             "VLM per optimizer step (0 = all local images)")
    parser.add_argument("--vlm_question_mode", type=str, default="pairwise",
                        choices=["target", "pairwise"],
                        help="target: ask whether the image has the requested class "
                             "(correct answer Yes). pairwise: ask that positive-Yes "
                             "question plus a cycling non-target question whose correct "
                             "answer is No, balancing the two answer directions")
    parser.add_argument("--vlm_prompt_template", type=str, default=None,
                        help="optional binary question template containing "
                             "'{class_name}'; the model chat template and image token "
                             "are added automatically")
    parser.add_argument("--vlm_attn_implementation", type=str, default="sdpa",
                        choices=["sdpa", "eager", "flash_attention_2"],
                        help="Transformers attention backend; sdpa needs no extra package")
    parser.add_argument("--vlm_skip_smoke", action="store_true",
                        help="skip the startup VLM image-gradient plumbing check")
    parser.add_argument("--cond_probe", action="store_true",
                        help="log held-out torchvision resnet50 V2 top-1/top-5/logp on "
                             "the training batch at diag steps (the same weights "
